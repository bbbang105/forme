...
- 피드 정렬: status=read 탭에서 readAt DESC 정렬 (최근 읽은 순)
...
| `packages/web/src/app/api/feed/add-url/route.ts` | 피드 URL 수동 등록 API (preview/save 2모드, OG 메타 파싱, "직접 추가" 시스템 소스) |
| `packages/web/src/components/features/feed/add-url-dialog.tsx` | URL 직접 추가 다이얼로그 (2단계 미리보기 폼, INTEREST_OPTIONS 태그 토글, 카테고리 선택) |
| `packages/web/src/app/api/feed/crawl/route.ts` | SSE 수동 크롤 API |
| `packages/web/src/app/api/feed/sources/reorder/route.ts` | 즐겨찾기 소스 순서 배치 업데이트 |
...
| `packages/web/src/app/api/cron/feed/route.ts` | Cron 자동 크롤 + 푸시 알림 (verifyCronSecret 멀티유저 인증, 응답에 내부 상세 미노출) |
...
| 프로젝트 | 경로 | 참고 대상 |
|----------|------|-----------|
| study-admin | `/Users/hansangho/Desktop/study-admin` | Discord OAuth, RSS/피드, UXUI 전체, 디자인 시스템 |
