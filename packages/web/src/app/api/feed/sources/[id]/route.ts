...
export const PATCH = withTracing('PATCH /api/feed/sources/[id]', async (request, ctx) => {
  ...
  try {
    ...
    console.error('[PATCH /api/feed/sources/:id]', err);
    return NextResponse.json(
...
export const DELETE = withTracing('DELETE /api/feed/sources/[id]', async (_request, ctx) => {
  const { id } = await (ctx as { params: Promise<{ id: string }> }).params;
  // Auth first
  const supabase = await createClient();
  const {
  ...
  try {
    ...
    console.error('[DELETE /api/feed/sources/:id]', err);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
