 ...
 */
export const POST = withTracing('POST /api/feed/add-url', async (request) => {
  // ── Auth ──
  ...
  try {
    ...
    let [manualSource] = await db
      ...
      .from(feedSources)
      .where(and(eq(feedSources.userId, user.id), eq(feedSources.url, MANUAL_SOURCE_URL)))
      .limit(1);
    ...
    const [existing] = await db
      .select({id: feedItems.id, deletedAt: feedItems.deletedAt})
      .from(feedItems)
      .where(and(eq(feedItems.sourceId, sourceId), eq(feedItems.url, url)))
      .limit(1);
    ...
    if (existing?.deletedAt) {
      [inserted] = await db
        ...
        .where(eq(feedItems.id, existing.id))
        .returning();
    } else {
      [inserted] = await db
        .insert(feedItems)
    ...
    console.error('[POST /api/feed/add-url]', err);
