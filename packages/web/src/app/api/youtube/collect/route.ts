...
export const POST = withTracing('POST /api/youtube/collect', async (request: Request) => {
  ...
  const sources = await db.select().from(youtubeSources)
    .where(and(
      eq(youtubeSources.userId, user.id),
      eq(youtubeSources.isActive, true),
      inArray(youtubeSources.id, sourceIds),
    ));
  ...
  const existingItems = await db.select({ videoId: youtubeItems.videoId })
    .from(youtubeItems)
        ...
        try {
          ...
          if (newItems.length > 0) {
            ...
            if (filteredItems.length > 0) {
              await db.insert(youtubeItems).values(filteredItems).onConflictDoNothing();
