 ...
 */
export const GET = withTracing('GET /api/cron/feed', async (request) => {
  ...
  for (const userId of userIds) {
    ...
    console.info('[cron/feed] user complete', summary);
    ...
    if (totalNewItems > 0) {
      ...
      try {
        ...
        console.error('[cron/feed] push failed', userId, err);
      }
    }
    ...
    if (totalNewItems > 0) {
      try {
        const recentItems = await db
          ...
          .where(
            and(
              ...
              gte(feedItems.collectedAt, since),
              isNull(feedItems.deletedAt),
            ),
          )
          .orderBy(desc(feedItems.collectedAt))
          .limit(20);
        ...
        console.error('[cron/feed] discord failed', userId, err);
      }
    }
  }
