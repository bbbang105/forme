...
export const POST = withTracing('POST /api/video/add-url', async (request: Request) => {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const [inserted] = await db.transaction(async (tx) => {
          await tx.delete(videoItems)
            .where(and(eq(videoItems.userId, user.id), eq(videoItems.videoId, videoId)));

          return tx.insert(videoItems).values({
            userId: user.id,
            sourceId: null,
            videoId,
            title: meta.title,
    ...
    async cancel() {
      ...
      if (insertedItemId) {
        try {
          await db.delete(videoItems)
            .where(and(eq(videoItems.id, insertedItemId), eq(videoItems.status, 'summarizing')));
        } catch {
          // best-effort cleanup
        }
      }
