...
/** Upload an image file to /api/memo/image. Returns URL or null. */
async function uploadImage(file: File): Promise<string | null> {
  const formData = new FormData();
  formData.append('image', file);
  try {
    const res = await fetch('/api/memo/image', {method: 'POST', body: formData});
...
function getImageFiles(dt: DataTransfer): File[] {
  ...
  for (let i = 0; i < dt.files.length; i++) {
    const f = dt.files[i];
    if (ALLOWED_TYPES.includes(f.type) && f.size <= MAX_SIZE) files.push(f);
  }
  return files;
}
