...
interface YoutubeSourceBarProps {
...
function SortableChannelCard(props: {
  ...
  onToggleFavorite: (source: YoutubeSource) => void;
  onToggleActive: (source: YoutubeSource) => void;
  onToggleTag: (source: YoutubeSource, tag: string) => void;
  onDelete: (source: YoutubeSource) => void;
}) {
...
export function YoutubeSourceBar({ sources, onAdd, onDelete, onUpdate }: YoutubeSourceBarProps) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<YoutubeSource | null>(null);
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    const res = await fetch('/api/youtube/sources/reorder', {
      method: 'PUT',
  ...
  const handleToggleFavorite = useCallback(async (source: YoutubeSource) => {
    await fetch(`/api/youtube/sources/${source.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleToggleActive = useCallback(async (source: YoutubeSource) => {
    await fetch(`/api/youtube/sources/${source.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleToggleTag = useCallback(async (source: YoutubeSource, tag: string) => {
